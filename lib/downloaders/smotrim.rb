...
class SmotrimDownloader < VideoDownloader
  def self.can_download?(url)
    return :page if url =~ %r{smotrim\.ru/video/\d+}i
    false
  end
  ...
  def get_track_list(url)
    md = url.match(%r{video/(?<video_id>\d+)}i)
    ...
    max_res_playlist_url.path = max_res_playlist_url.path.gsub(%r{([^/]+?)$}, max_res_entry[:filename])
